...
const Navbar = () => {
  ...
  // Lock scroll and dismiss input focus on mobile when menu opens
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <>
      <motion.nav 
        ...
        animate={{ 
          y: (isInputFocused && !isOpen) ? -100 : 0, 
          opacity: (isInputFocused && !isOpen) ? 0 : 1 
        }}
          ...
          <div className={`flex justify-between items-center transition-all duration-700 ${scrolled ? 'h-16' : 'h-24'}`}>
            ...
            <div className="md:hidden">
              <button
                ...
                className="p-3 rounded-2xl glass border-white/10 text-white transition-all active:scale-90 animate-none pointer-events-auto"
      ...
      {/* Mobile Drawer (Direct relative of viewport to fully bypass transform containing block bugs on iOS/Android Safari) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 z-[9999] bg-black/98 backdrop-blur-3xl overflow-hidden"
          >
            <div className="absolute top-8 right-8">
               <button onClick={() => setIsOpen(false)} className="p-4 rounded-full glass border-white/10 text-white active:scale-90">
                 <X className="w-8 h-8" />
               </button>
            </div>
              ...
              <div className="pt-12 mt-auto space-y-6">
                {user ? (
                   <div className="grid grid-cols-2 gap-4">
                    ...
                    <button onClick={() => { setIsOpen(false); signOut(); }} className="p-6 rounded-3xl glass border-white/10 flex flex-col gap-4">
