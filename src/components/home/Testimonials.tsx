  ...
  return (
    <section id="testimonials" className="py-32 md:py-48 relative overflow-hidden bg-[#0a0a0c]/40">
      ...
      <div className="relative flex overflow-hidden group py-24 -my-24">
        ...
        <div
          className={`flex gap-6 md:gap-12 w-max px-6 ${!shouldReduceGfx ? 'animate-marquee marquee-pause' : ''}`}
        >
          {duplicatedTestimonials.map((t: any, i: number) => (
            <motion.div
              ...
              className="w-[320px] md:w-[550px] shrink-0 glass-card p-8 md:p-12 border-white/5 transition-all duration-700 group/card relative rounded-[2.5rem] md:rounded-[3rem]"
            >
              ...
              <div className="flex items-start justify-between mb-8 md:mb-10 relative z-10">
                <div className="flex items-center gap-6 md:gap-8">
                  <div className="w-16 h-16 md:w-24 md:h-24 rounded-[1rem] md:rounded-[1.5rem] overflow-hidden border-2 border-[var(--c-1-start)]/20 bg-zinc-900 shrink-0 rotate-3 group-hover/card:rotate-0 transition-all duration-700 relative shadow-2xl">
                    {t.imageUrl ? (
                      <OptimizedImage 
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-zinc-800">
                        <Users className="w-8 h-8 md:w-12 md:h-12 text-zinc-600" />
                      </div>
                    )}
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-xl md:text-2xl font-bold text-white tracking-tighter mb-1 md:mb-2">{t.name}</h4>
                    <p className="text-[8px] md:text-[10px] font-mono font-black text-[var(--c-1-start)] uppercase tracking-[0.3em] md:tracking-[0.4em]">{t.role}</p>
                  </div>
                </div>
                <Quote className="w-8 h-8 md:w-12 md:h-12 text-[var(--c-1-start)]/10 group-hover/card:text-[var(--c-1-start)]/40 transition-colors duration-700" />
              </div>
              <p className="text-zinc-400 text-base md:text-lg leading-relaxed italic font-light relative z-10 tracking-tight font-serif line-clamp-4 md:line-clamp-none">
                &quot;{t.message}&quot;
              </p>
              
              <div className="mt-8 md:mt-10 flex gap-1 relative z-10">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="w-3 h-3 md:w-4 md:h-4 fill-[var(--c-1-start)] text-[var(--c-1-start)] opacity-50 group-hover:opacity-100 transition-opacity duration-700" />
                ))}
              </div>
            </motion.div>
          ))}
