...
export async function POST(req: Request) {
  try {
    ...
    let emailList: Array<{ email: string; name: string; class?: string; section?: string; roll?: string }> = [];
    ...
    if (targetType === 'individual') {
      ...
      const { data: member } = await supabaseAdmin
        .from('member')
        .select('full_name, class, section, roll')
      ...
      const mergedMap = new Map<string, { email: string; name: string; class: string; section: string; roll: string }>();
      ...
      const { data: members } = await supabaseAdmin
        .from('member')
        .select('email, email_address, full_name, class, section, roll');
